...
**Resolved in task foundry-04 — one photo per pasted enquiry.** `POST /api/enquiries` takes an optional
`imageDataUrl` (`data:image/{jpeg,png,webp};base64,...`, at most 2 MB decoded, else 400) riding the
same JSON body; with a photo the body may be blank. `QuoteDesk.Intake.PastedImage` validates it and
`PasteAdapter.FromPastedTextAndImage` attaches it as an `EnquiryAttachment` carrying `DataUrl`. The
status rule is now: blank body **and no readable image** (an image attachment whose content is
present) → `needs_manual_entry` — so a voice note, or an attachment known only by metadata, still
needs a human. The image is stored on `Enquiries.ImageDataUrl` (it arrives on one request and is read
on `/process`, and Retry must read it again), passed into the workflow on `EnquiryInput.ImageDataUrl`,
sent to Intake as a `DataContent` alongside the wrapped text, and **stripped by `IntakeExecutor`** so
no checkpoint after Intake carries it (integration-tested by scanning every checkpoint payload). It is
not in `EnquiryDetailResponse`. The web client downscales to ≤1280px JPEG at 0.8 on a canvas and keeps
the photo in memory only, never in `sessionStorage`. A line whose quantity Intake cannot read is
written as quantity 0; `ResolveExecutor.ReconcileAsync` turns any quantity ≤ 0 into an unresolved line
in code, so it is never priced. The provider here is Foundry, not Gemini.

**Also resolved in foundry-04, after live tests with a real handwritten, mixed Gujarati-English photo:**

- **Intake's model depends on the input.** `Llm:IntakeModel` (`gpt-5-nano`) reads typed text;
  `Llm:IntakeImageModel` (`gpt-5-mini`) reads an enquiry with a photo, falling back to `IntakeModel`.
  On the real photo, nano got one quantity and no full specification right and invented a qualifier;
  mini read far more correctly. Typed text stays on nano, which still makes both worked-example
  judgement calls correctly. The trace's `StageEvent.Model` names the model that actually read it.
- **The approval card shows the photo** (`GET /api/enquiries/{id}/image`, behind the fallback policy,
  `no-store` and `nosniff`, re-validated by `PastedImage.TryDecode` before serving). This is the check
  for a *confident* misread: mini read a handwritten "20mm" as "25mm", a real SKU, so every code check
  passed and the line was priced. Reading handwriting is not reliable even for the largest models —
  a handwritten "2₹" (meaning 2RS) was read as "2Z" by several models, including Claude Opus — so the
  design relies on the human comparing the photo to the lines, not on a better reader. A second
  "re-read the photo with the catalogue's valid options" agent loop was considered and deliberately
  not built: the misreads seen were systematic (a re-read repeats them), it would add a model call per
  problem line, and it would not catch a confident misread at all.
- **Unreadable quantity:** the guard in `ResolveExecutor.ReconcileAsync` checks Intake's own quantity,
  not only Resolve's, and `resolve.md` forbids filling a quantity in, so a quantity the customer never
  wrote can never be priced.
- **Review hardening:** `POST /api/enquiries` has a 3.5 MB request limit; image bytes must match the
  declared type; `verify_catalogue_term` suggests only for terms of at most 4 words / 64 characters,
  over a bounded beam, and honours cancellation.
- **Accepted for the single-tenant demo, and to fix before any public deployment:** any signed-in user
  can fetch any enquiry's photo by id (the same access model `GET /api/enquiries/{id}` and
  `GET /api/approvals` already have, but a photo can carry more personal data); there is no per-user cap
  on stored uploads; `GetByIdAsync` still loads the image column on every read.
...
**Extended in task foundry-04 — misspellings and family names.** `CatalogueTermCheck` gained
`Suggestions[≤3]`: for a term that is not known, the closest real catalogue phrases, each unknown word
swapped for a vocabulary word within a small edit distance (1 for words up to 5 letters, 2 beyond), kept
only if every word of the reading appears together in one item ("PV belt" → "pu belt", never "pu
bearing"). The vocabulary is built from item names, families and attributes — **never SKU codes** — and
a word containing a digit is never corrected, since "6211" one edit from "6201" is a different part, not
a typo. Matching now reads the whole catalogue (`ICatalogRepository.GetAllAsync`, a few hundred rows)
instead of a SKU/name substring recall, which is what makes a family name such as "SpindleTapes" known.
