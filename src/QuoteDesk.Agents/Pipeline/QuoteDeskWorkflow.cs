...
public sealed record WorkflowNodes(IntakeExecutor Intake, ResolveExecutor Resolve, PriceExecutor Price, ApproveExecutor Approve);

/// <summary>
/// Assembles the fixed pipeline graph: <c>Intake → Resolve → Price → [approval RequestPort] →
...
public static class QuoteDeskWorkflow
{
    ...
    public static Workflow Build(WorkflowNodes nodes)
    {
        ...
        return new WorkflowBuilder(nodes.Intake)
            .AddEdge(nodes.Intake, nodes.Resolve)
